...
interface ImprovementRow { subject_id: string; flags: Record<string, boolean>; remark: string | null; }
...
function TeacherStudentRecordsContent() {
  ...
  function load() {
    ...
    fetch(`/api/teacher/students/${id}/records?termId=${termId}`).then((r) => r.json()).then((d) => {
      ...
      (d.improvementAreas || []).forEach((row: ImprovementRow) => {
        areaMap[row.subject_id] = row.flags || {};
        remarkMap[row.subject_id] = row.remark || "";
      });
      setImprovementAreas(areaMap);
      setSubjectRemarks(remarkMap);
              ...
              <div key={subjectId} className="bg-white rounded-xl shadow-sm p-5">
                ...
                <div className="mb-4">
                  ...
                  <label className="text-xs font-medium text-slate-500 flex items-center gap-1 mb-1">
                    <MessageSquare className="w-3.5 h-3.5" /> Teacher's remark for {subjectName}
                  </label>
                  <textarea
                    value={subjectRemarks[subjectId] || ""}
                    onChange={(e) => setSubjectRemarks((prev) => ({ ...prev, [subjectId]: e.target.value }))}
                    rows={2}
                    placeholder={`Add a remark specific to this student's ${subjectName} performance…`}
                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm mb-2"
                  />
                  <button
                    onClick={() => saveSection(`area-${subjectId}`, { improvementArea: { subjectId, flags: areas, remark: subjectRemarks[subjectId] || "" } })}
                    disabled={saving === `area-${subjectId}`}
                    className="bg-brand-teal text-white rounded-lg px-3 py-1 text-xs font-semibold hover:opacity-90 transition-opacity disabled:opacity-50"
                  >
                    {saving === `area-${subjectId}` ? "Saving…" : saved === `area-${subjectId}` ? "Saved" : "Save Areas & Remark"}
                  </button>
                </div>
