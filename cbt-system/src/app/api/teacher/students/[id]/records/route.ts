...
// A teacher can view a student's class-wide records (remarks, attendance)
// if they're assigned to that class for at least one subject. Admins
// always pass.
...
// A teacher can edit subject-scoped records (improvement areas, the
// subject remark, projects) only for a subject they're actually assigned
// to teach in that class — not just any subject in that class. This is
// the enforcement that was missing: a teacher assigned to Mathematics in
// a class could previously edit English's improvement areas for the same
// student, just because they had some assignment to that class.
async function canAccessSubject(classId: string, subjectId: string, staffId: string, isAdmin: boolean) {
  if (isAdmin) return true;
  const supabase = createAdminSupabase();
  const { data } = await supabase
    .from("teacher_assignments")
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const [{ data: remarks }, { data: attendance }, { data: improvementAreas }, { data: projects }, { data: classSubjects }] =
    await Promise.all([
  ...
  // Only show subjects this teacher is actually assigned to teach in this
  // class — showing an edit form for a subject that would then 403 on
  // save is a worse experience than not showing it at all. Admins see
  // every subject in the class regardless.
  ...
  if (staff.role !== "admin") {
    ...
    const allowedSubjectIds = new Set((myAssignments || []).map((a) => a.subject_id));
    subjects = subjects.filter((s) => allowedSubjectIds.has(s.subject_id));
  }
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (improvementArea) {
    if (!(await canAccessSubject(student.class_id, improvementArea.subjectId, staff.id, staff.role === "admin"))) {
      return NextResponse.json({ error: "Not assigned to this subject for this class." }, { status: 403 });
    }
    const { error } = await supabase.from("improvement_areas").upsert(
      {
        student_id: params.id, term_id: termId, subject_id: improvementArea.subjectId,
        flags: improvementArea.flags ?? {}, remark: improvementArea.remark ?? null,
  ...
  if (project) {
    if (!(await canAccessSubject(student.class_id, project.subjectId, staff.id, staff.role === "admin"))) {
      return NextResponse.json({ error: "Not assigned to this subject for this class." }, { status: 403 });
    }
