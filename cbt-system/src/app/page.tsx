...
export default function LandingPage() {
  return (
    <main className="min-h-screen bg-gradient-to-b from-brand-navyDeep via-brand-navy to-[#0d2444] text-white overflow-hidden relative">
      ...
      {/* Animated gradient mesh — slow-drifting radial blobs, pure CSS
          background-position animation. Cheap: one layer, no JS, no
          images. Adds depth without the weight of a 3D/WebGPU approach. */}
      <div
        className="pointer-events-none absolute inset-0 opacity-40 animate-gradientDrift"
        style={{
          backgroundImage:
            "radial-gradient(circle at 20% 30%, rgba(212,168,67,0.25), transparent 45%), radial-gradient(circle at 80% 70%, rgba(0,123,131,0.25), transparent 45%), radial-gradient(circle at 50% 100%, rgba(232,121,43,0.15), transparent 50%)",
          backgroundSize: "200% 200%"
        }}
      />

      {/* Subtle dot-grid texture for tactile depth, barely visible */}
      <div
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
        ...
        <KeyRound className="absolute top-[65%] left-[6%] w-10 h-10 text-brand-teal/15 animate-floatSlow" style={{ animationDelay: "0.8s" }} />
        <Timer className="absolute top-[8%] right-[28%] w-9 h-9 text-brand-coral/15 animate-floatSlowReverse" style={{ animationDelay: "1.5s" }} />
        ...
        <div className="absolute top-[80%] left-[30%] w-1.5 h-1.5 rounded-full bg-brand-coral/30 animate-floatSlow" style={{ animationDelay: "2.4s" }} />
      </div>

      <div className="max-w-6xl mx-auto px-6 py-16 relative">
        <header className="text-center mb-16 animate-fadeIn">
          <div className="relative inline-flex items-center justify-center w-24 h-24 mb-5">
            <div className="absolute inset-0 rounded-full bg-brand-gold/40 blur-xl animate-glowPulse" />
            <div className="relative inline-flex items-center justify-center w-24 h-24 rounded-full bg-white shadow-xl ring-4 ring-white/10 p-2">
          ...
          <div
            className="w-14 h-1.5 rounded-full mx-auto mt-4 mb-4 animate-shimmer"
            style={{
              backgroundImage: "linear-gradient(90deg, #D4A843 0%, #F5D98A 50%, #D4A843 100%)",
